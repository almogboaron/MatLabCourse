...
properties (Access = public)
  ...
  % =============================
	...
	function pt = release_patient(h,first_name,last_name)
		...
		h.N_patients = h.N_patients - 1;
        pt.physician.remove_patient_from_physician_list(pt);
	end
    ...
    function disp_hospital_balance(h)
        Salleries = h.N_physicians*1500;
        Income_bills = 0;
        for i=1:numel(h.patient_list)
            if isa(h.patient_list{i},'Patient')
                Income_bills = Income_bills +sum([h.patient_list{i}.bill]);
            end
        end
        balance = Income_bills - Salleries ;
        fprintf('The hostpital balance is %d \n',balance)
    end
	% --------------------------------
	% Plotting
	% --------------------------------
    ...
    function allocate_physician_to_patient(h, pt_first_name, pt_last_name, ph_first_name, ph_last_name)
        ...
        ph.add_patient_to_physician_list(pt)
        pt.physician= ph;
    end% public methods
      end
  % ============================
